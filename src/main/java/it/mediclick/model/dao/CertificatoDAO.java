...
public class CertificatoDAO
{
	...
	public void insert(Certificato c) throws SQLException
	{
		...
		try
		{
			...
			_contex.eseguiUpdate(sql, medicoId, tipoCertificatoId, c.getNomeFile(), c.getDatiDocumento(), stato, c.getMimeType(), c.getDataCaricamento(), c.getDataScadenza(), approvatoreId);
		}
		catch (SQLException e)
		{
			e.printStackTrace();
	...
	public void updateStato(int id, Certificato.Stato stato, int approvatoreId) throws SQLException
	{
		String sql = """
						   UPDATE Certificato
						   SET Approved_by = ?, Stato = ?
						   WHERE ID = ?
						""";

		try
		{
			_contex.eseguiUpdate(sql, approvatoreId, stato.getLabel(), id);
		}
		catch (SQLException e)
		{
			throw new SQLException("Errore nell' aggiornamento dello stato del certificato: ", e);
