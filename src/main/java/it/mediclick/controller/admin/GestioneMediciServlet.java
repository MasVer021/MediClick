...
@WebServlet("/admin/medici")
public class GestioneMediciServlet extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		try
		{
			List<Medico> medici = amministrazioneService.getTuttiIMedici();
			...
			request.getRequestDispatcher("/WEB-INF/view/admin/medici.jsp").forward(request, response);
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		try
		{
			...
			boolean blocca = ValidationUtils.parseBoolean(request.getParameter("blocca"), "blocca");

			amministrazioneService.bloccaUtente(medicoId, blocca);

			String azione = blocca ? "bloccato" : "sbloccato";
			request.getSession().setAttribute("successo", "Account medico ID " + medicoId + " " + azione + " con successo!");

			response.sendRedirect(request.getContextPath() + "/admin/medici");
		}
		catch (AmministratoreException e)
		{
