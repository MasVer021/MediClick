			...
			<div class="mc-search-layout">
				<aside class="mc-search-layout__sidebar">
					<div class="mc-card mc-card--primary">
						...
						<form method="get" action="<c:url value='/search'/>" class="mc-card__body mc-flex-col mc-gap-md">
							<div class="mc-form-group">
								<label class="mc-label" for="specialista">Seleziona la categoria</label> <select class="mc-select" id="specialista" name="specialista">
							...
							<div class="mc-form-group" id="medici-input-search-group">
								<label class="mc-label" for="querySpecialista">Nome Specialista</label> <input class="mc-input" type="search" placeholder="Es. Mario Rossi" id="querySpecialista" name="querySpecialista" autocomplete="off">
							</div>
							<div class="mc-form-group" id="citta-input-search-group">
								<label class="mc-label" for="citta">Citt&agrave;</label> <input class="mc-input" type="text" id="citta" name="citta" placeholder="Roma" autocomplete="off">
							</div>
								...
								<div class="mc-card__avatar-box">
									<c:choose>
										<c:when test="${not empty m.medico.fotoprofiloBase64}">
											<img class="mc-image-preview" src="data:image/jpeg;base64,${m.medico.fotoprofiloBase64}" alt="Foto Dr. ${m.medico.nome} ${m.medico.cognome}" />
										</c:when>
										<c:otherwise>
								...
								<div class="mc-card__body mc-p-md mc-flex-col mc-gap-sm mc-justify-between">
									<div>
										...
										<div class="mc-flex-row mc-justify-between mc-align-center mc-mb-sm">
											<span class="mc-badge mc-badge--info">${m.categoria.nome}</span> <span class="mc-text-bold">&#9733; ${m.valoreRecensioni}/5 (${m.numeroRecensioni})</span>
										</div>
									...
									<a href="<c:url value='/profiloMedico?id=${m.medico.id}'/>" class="mc-btn mc-btn--primary mc-btn--block mc-mt-sm"> Prenota - ${m.costo} &euro; </a>
								</div>
							</div>
						</c:forEach>
					</div>
		...
		<c:otherwise>
			<div class="mc-card mc-card--small mc-card--primary">
				...
				<form method="get" action="<c:url value='/search'/>" class="mc-card__body mc-flex-col mc-gap-md">
					...
					<div class="mc-form-group" id="citta-input-search-group">
						<label class="mc-label" for="citta">Citt&agrave;</label> <input class="mc-input" type="text" id="citta" name="citta" placeholder="Roma" list="citta-suggest" autocomplete="off">
					</div>
