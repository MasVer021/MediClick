		...
		<div class="mc-card__body">
			<form action="${pageContext.request.contextPath}/admin/impostazioni" method="post">
				<div class="mc-form-group">
					<label class="mc-label" for="chiave">Chiave Parametro *</label> <input class="mc-input" type="text" id="chiave" name="chiave" required placeholder="es. tasse_piattaforma">
				</div>
				<div class="mc-form-group">
					<label class="mc-label" for="valore">Nuovo Valore *</label> <input class="mc-input" type="text" id="valore" name="valore" required placeholder="es. 15">
				</div>
				<button class="mc-btn mc-btn--outline mc-btn--block mc-mt-md" type="submit">Salva Impostazione</button>
			</form>
		</div>
	</div>
			...
			<table class="mc-table mc-table--zebra">
				...
				<tbody>
					<c:forEach items="${impostazioni}" var="imp">
						<tr>
							<td><span class="mc-text-bold"> <c:out value="${imp.chiave}" />
							</span></td>
					...
					<c:if test="${empty impostazioni}">
						<tr>
							<td colspan="4" class="mc-text-center mc-text-muted mc-p-lg">Nessun parametro impostato nel sistema.</td>
						</tr>
					</c:if>
				</tbody>
			</table>
