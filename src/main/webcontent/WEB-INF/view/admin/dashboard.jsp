		...
		<main class="mc-container mc-container-ver mc-mb-xl">
			...
			<div class="mc-kpi-grid">
				<div class="mc-kpi-card">
					<span class="mc-kpi-card__title">Medici Registrati</span>
					<span class="mc-kpi-card__value">${stats.totaleMedici}</span>
				</div>


				<div class="mc-kpi-card ">
					<span class="mc-kpi-card__title">Medici da Approvare</span>
					<span class="mc-kpi-card__value">${stats.mediciDaApprovare}</span>
				</div>


				<div class="mc-kpi-card">
					<span class="mc-kpi-card__title">Ricavi Totali Piattaforma</span>
					<span class="mc-kpi-card__value">&euro;
						<fmt:formatNumber value="${stats.guadagniPiattaforma}" pattern="#,##0.00" />
					</span>
				</div>
			</div>
								...
								<tbody>
									<c:forEach items="${mediciInAttesa}" var="medico">
										<tr>
											<td>${medico.id}</td>
											<td>
												<c:out value="${medico.cognome}" />
											</td>
											<td>
												<c:out value="${medico.nome}" />
											</td>
											<td>
												<c:out value="${medico.pIva}" />
											</td>
											<td><span class="mc-badge mc-badge--warning">${medico.statoVerifica}</span>
											</td>
											...
											<c:forEach items="${tipiCertificato}" var="tc">
												<td>
													...
													<c:set var="certificatoTrovato" value="" />
													<c:forEach
													...
													<c:choose>
														<c:when test="${trovato}">
															...
															<a href="${pageContext.request.contextPath}/admin/downloadCertificato?id=${certificatoTrovato.id}"
															   target="_blank" class="cert-link"
															   title="Visualizza ${certificatoTrovato.nomeFile}">

																<c:if test="${certificatoTrovato.stato == 'RIFIUTATO'}">
																	<c:set var="badge" value="danger" />
																</c:if>

																<c:if test="${certificatoTrovato.stato == 'APPROVATO'}">
																	<c:set var="badge" value="success" />
																</c:if>

																<span class="mc-badge mc-badge--${badge}">
																	<c:out value="${certificatoTrovato.nomeFile}" />
																</span>
															</a>
															<c:choose>
																<c:when
																	...
																	<div class="mc-table-action-cell mc-mt-sm">
																		...
																		<form method="post"
																			...
																			<input type="hidden" name="certificatoId"
																				   value="${certificatoTrovato.id}">
																			<input type="hidden" name="approvato"
																				   value="true">
																			<button type="submit"
																					class="mc-btn mc-btn--success mc-btn--sm"
											...
											<td>
												<div class="mc-table-action-cell">
													<form method="post"
														...
														<button type="submit"
																class="mc-btn mc-btn--success mc-btn--sm">Approva</button>
													</form>
													...
													<form method="post"
														...
														<button type="submit"
																class="mc-btn mc-btn--danger mc-btn--sm">Rifiuta</button>
													</form>
												</div>
											</td>
										</tr>
