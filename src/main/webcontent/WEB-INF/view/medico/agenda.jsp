				...
				<div class="mc-card__header ">
					...
					<form action="${pageContext.request.contextPath}/medico/agenda" method="GET">
						<div class="mc-form-group">
							<label class="mc-label" for="data">Seleziona Data</label>
							<input class="mc-input" type="date" id="data" name="data" value="${dataMostrata}" required>
						</div>

						<button class="mc-btn mc-btn--primary" type="submit">Filtra</button>

						<a class="mc-btn mc-btn--outline" href="${pageContext.request.contextPath}/medico/agenda">Vai a
							Oggi</a>
					</form>
				</div>
						...
						<c:otherwise>
							<div class="mc-grid mc-grid-3">
								<c:forEach var="slot" items="${agenda}">
									...
									<div
										 class="mc-card mc-card--generic ${slot.statoSlot == 'Disponibile' ? 'mc-card--libera' : ''}">

										<div class="mc-card__header">
											<h2 class="mc-card__title mc-mb-none" style="display:inline">
												<fmt:parseDate value="${slot.dataOraInizio}"
															   pattern="yyyy-MM-dd'T'HH:mm" var="parsedInizio"
															   type="both" />
												<fmt:formatDate value="${parsedInizio}" pattern="HH:mm" />
												-
												<fmt:parseDate value="${slot.dataOraFine}" pattern="yyyy-MM-dd'T'HH:mm"
															   var="parsedFine" type="both" />
										...
										<div class="mc-card__body">
											<c:choose>
												<c:when
														test="${slot.statoSlot == 'Prenotata' || slot.statoSlot == 'Completata'}">
													<div class="mc-flex-col mc-mg-sm">
														<span class="mc-text-muted mc-font-xs">Paziente</span>
														<span class="mc-text-bold mc-font-md">${slot.nomePaziente}
															${slot.cognomePaziente}</span>
													</div>
													<div class="mc-flex-col">
														<span class="mc-text-muted mc-font-xs">Prestazione</span>
														<span
															  class="mc-text-bold mc-font-md">${slot.nomePrestazione}</span>
													</div>
													<div class="mc-flex-col">
														<span class="mc-text-muted mc-font-xs">Telefono</span>
														<span
															  class="mc-text-bold mc-font-md">${slot.telefonoPaziente}</span>
													</div>
												</c:when>
												<c:otherwise>
										...
										<div class="mc-card__footer">
											<c:choose>
												<c:when test="${slot.statoSlot == 'Prenotata'}">
													<form action="${pageContext.request.contextPath}/medico/agenda"
														  method="POST">
														<input type="hidden" name="action" value="completa">
														<input type="hidden" name="prenotazioneId"
															   value="${slot.prenotazioneId}">
														<input type="hidden" name="data" value="${dataMostrata}">
														<button class="mc-btn mc-btn--success" type="submit">Concludi
															Visita</button>
													</form>
													<form action="${pageContext.request.contextPath}/medico/agenda"
														  method="POST">
												...
												<c:otherwise>
													<form action="${pageContext.request.contextPath}/medico/agenda"
														...
														<input type="hidden" name="disponibilitaId"
															   value="${slot.disponibilitaId}">
														<input type="hidden" name="data" value="${dataMostrata}">
														<button class="mc-btn mc-btn--danger" type="submit">Rimuovi
															Slot</button>
													</form>
												</c:otherwise>
