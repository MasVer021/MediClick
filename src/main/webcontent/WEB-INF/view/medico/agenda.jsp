		                   ...
		                   <div class="mc-card mc-card--generic ${slot.statoSlot == 'Disponibile' ? 'mc-card--libera' : ''}">
		                        ...
		                        <div class="mc-card__header">
		                        	<h2 class="mc-card__title mc-mb-none" style="display:inline">
			                            ...
			                            <c:if test="${slot.statoSlot == 'Prenotata'}"><c:set var="badgeClass" value="mc-badge--info" /></c:if>
			                            <c:if test="${slot.statoSlot == 'Completata'}"><c:set var="badgeClass" value="mc-badge--success" /></c:if>
		                            ...
		                            <c:choose>
		                                <c:when test="${slot.statoSlot == 'Prenotata' || slot.statoSlot == 'Completata'}">
			                                <div class="mc-flex-col mc-mg-sm">
									            ...
									            <span class="mc-text-bold mc-font-md">${slot.nomePaziente} ${slot.cognomePaziente}</span>
									        </div>
									        <div class="mc-flex-col">
									            <span class="mc-text-muted mc-font-xs">Prestazione</span>
									            <span class="mc-text-bold mc-font-md">${slot.nomePrestazione}</span>
									        </div>
									        <div class="mc-flex-col">
									            <span class="mc-text-muted mc-font-xs">Telefono</span>
									            <span class="mc-text-bold mc-font-md">${slot.telefonoPaziente}</span>
									        </div>
		                                </c:when>
		                                <c:otherwise>
