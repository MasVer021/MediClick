				...
				<div class="mc-card__body">
					<form action="${pageContext.request.contextPath}/medico/profilo" method="POST"
						...
						<div class="mc-form-group">
							...
							<input class="mc-input" type="text" id="cognome" name="cognome" value="${medico.cognome}"
								   required>
						</div>
						<div class="mc-form-group">
							<label class="mc-label" for="pIva">Partita IVA</label>
							<input class="mc-input" type="text" id="pIva" name="pIva" value="${medico.pIva}" required
								   maxlength="11">
						</div>
						<div class="mc-form-group">
						...
						<div class="mc-form-group">
							...
							<textarea class="mc-textarea" id="bio" name="bio" rows="4"
									  cols="50"><c:out value="${medico.bio}"/></textarea>
						</div>
						...
						<div>
							<c:choose>
								<c:when test="${not empty medico.fotoprofilo}">
									<img class="medico-avatar" src="data:image/jpeg;base64,${medico.fotoprofiloBase64}"
										 alt="Foto Dr. ${medico.nome} ${medico.cognome}" />
								</c:when>
								<c:otherwise>
					...
					<form action="${pageContext.request.contextPath}/medico/profilo" method="POST"
						...
						<div class="mc-form-group">
							...
							<div class="mc-photo-upload">
								...
								<input type="file" id="fotoprofilo" name="foto" class="mc-file-input"
									   accept="image/*">
							</div>
						</div>
						...
						<tbody>
							<c:forEach var="erogazione" items="${miePrestazioni}">
								<tr>
									...
									<td>
										<fmt:formatNumber value="${erogazione.prezzoLordoListino}" type="currency"
														  currencySymbol="&euro;" />
									</td>
				...
				<div class="mc-card__footer mc-flex-col">
					...
					<form action="${pageContext.request.contextPath}/medico/profilo" method="POST"
						...
						<div class="mc-form-group">
							...
							<input class="mc-input" type="number" id="durata" name="durata" min="30" step="30"
								   placeholder="30" required>
						</div>

						<div class="mc-form-group">
							<label class="mc-label" for="prezzo">Prezzo Lordo (&euro;)</label>
							<input class="mc-input" type="number" id="prezzo" name="prezzo" step="0.01" min="0"
								   placeholder="Es. 80.00" required>
						</div>
