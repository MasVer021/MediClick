						...
						<tbody>
							<c:forEach items="${certificatiCaricati}" var="cert">
								<tr>
									...
									<td>
										<fmt:parseDate value="${cert.dataCaricamento}" pattern="yyyy-MM-dd'T'HH:mm"
													   var="parsedCaricamento" type="both" />
										<fmt:formatDate value="${parsedCaricamento}" pattern="dd/MM/yyyy HH:mm" />
									</td>
									<td>
										<c:choose>
											<c:when test="${not empty cert.dataScadenza}">
												<fmt:parseDate value="${cert.dataScadenza}" pattern="yyyy-MM-dd'T'HH:mm"
															   var="parsedScadenza" type="both" />
									...
									<td>
										<div class="mc-table-action-cell">
											<form action="${pageContext.request.contextPath}/medico/certificati"
												  method="POST">
												<input type="hidden" name="action" value="elimina-certificato">
												<input type="hidden" name="certificatoId" value="${cert.id}">
												<button class="mc-btn mc-btn--danger mc-btn--sm" type="submit"
														onclick="return confirm('Sei sicuro di voler eliminare questo certificato?');">
													Elimina
												</button>
											</form>
										</div>
						...
						<form action="${pageContext.request.contextPath}/medico/certificati" method="POST"
							...
							<div class="mc-form-group">
								...
								<div class="mc-photo-upload"
									 style="margin: 0.5rem 0; align-items: flex-start; gap: 0.5rem;">
									<label for="documento" class="mc-btn-upload">Scegli File PDF</label>

									<input type="file" id="documento" name="documento" class="mc-file-input"
