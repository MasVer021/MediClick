										...
										<td>
											<div class="mc-table-action-cell">
												<c:if test="${p.stato.label == 'Confermata' && p.futura}">
													<form action="<c:url value='/paziente/prenotazioni'/>" method="post"
														  onsubmit="return confirm('Sei sicuro di voler disdire questa visita?');"
														  style="margin:0;">
														<input type="hidden" name="action" value="disdici">
														<input type="hidden" name="prenotazioneId" value="${p.id}">
														<button type="submit"
																class="mc-btn mc-btn--danger mc-btn--sm">Disdici</button>
													</form>
												</c:if>

												<c:if
													  test="${(p.stato.label == 'Confermata' || p.stato.label == 'Erogata') && not p.futura}">
													<a href="<c:url value='/paziente/recensione?prenotazioneId=${p.id}'/>"
													   class="mc-btn mc-btn--secondary mc-btn--sm">Recensisci</a>
												</c:if>
											</div>
										</td>

										<td>
											<a href="<c:url value='/paziente/fattura?prenotazioneId=${p.id}'/>"
											   class="mc-btn mc-btn--secondary mc-btn--sm">visualizza</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
